...
template<typename T>
concept HasMemory = requires(T t, ByteResult b, HalfwordResult h, WordResult w, WriteResult v) {
    ...
    v = t.Write8(Address{}, u8{});   // Writes a byte to an address.
    v = t.Write16(Address{}, u16{}); // Writes a halfword to an address.
    v = t.Write32(Address{}, u32{}); // Writes a word to an address.
};

// T implements the fetch cycle.
template<typename T>
concept HasFetch = requires(T t, Address a, WordResult w) {
